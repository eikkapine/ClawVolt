...
![License](https://img.shields.io/badge/MIT-00ff88?style=for-the-badge&logoColor=white)

<br/>

<img src="https://img.shields.io/badge/-%E2%9A%A1%20Dynamic%20Voltage%20%E2%80%A2%20%F0%9F%AA%B5%20Crash%20Logger%20%E2%80%A2%20%F0%9F%96%A5%EF%B8%8F%20GUI%20%2B%20CLI%20%E2%80%A2%20%F0%9F%94%A5%20ADLX%20Native-1a1a2e?style=flat-square&labelColor=0f3460&color=00ccff" height="28"/>
...
### Why a compiled C++ bridge?

ADLX is a native C++ SDK. Python can't call it directly. The bridge wraps every ADLX operation into a single subprocess with clean machine-parseable output:
...
| 🎛 **Threshold editor** | Add/remove/edit rows live with color-coded dot indicators per threshold |
...
| Feature | Detail |
|:---|:---|
| ✈️ **Flight recorder** | 300-sample circular buffer (~150s) of clock + voltage at every poll tick |
| 👁 **Event Log watcher** | Background thread polls for TDR (4101), kernel crash (41), bad shutdown (6008) |
| 🧠 **Crash analysis** | 7 reason codes with pattern-matched telemetry classification |
| 📝 **Structured reports** | Full crash report: reason, analysis, config snapshot, voltage history, telemetry table, recommendations |
...
│
├── 📁 build/
│   ├── build_exe_py312.bat        ◄ one-click exe builder (run from repo root)
...
|:---|:---:|:---|
| ![Windows](https://img.shields.io/badge/Windows%2010%2F11-0078D6?style=flat-square&logo=windows) | 64-bit | — |
| ![AMD](https://img.shields.io/badge/RX%209070%20XT-ED1C24?style=flat-square&logo=amd) | RDNA4 | RDNA3/2 may work via MGT1 path |
| ![Adrenalin](https://img.shields.io/badge/AMD%20Adrenalin%20Driver-ED1C24?style=flat-square&logo=amd) | Latest | Installs `ADLX.dll` automatically |
| ![Python](https://img.shields.io/badge/Python-3.12-3776AB?style=flat-square&logo=python) | **3.12 specifically** | 3.14+ breaks PyInstaller exe builds |
| ![VS](https://img.shields.io/badge/Visual%20Studio%202022-5C2D91?style=flat-square&logo=visualstudio) | 2019 or 2022 | C++ Desktop workload required |
| ![CMake](https://img.shields.io/badge/CMake-3.16+-064F8C?style=flat-square&logo=cmake) | 3.16+ | For building the C++ bridge |
| ![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) | Any | For cloning the ADLX SDK |
| ![ADLX](https://img.shields.io/badge/AMD%20ADLX%20SDK-ED1C24?style=flat-square&logo=github) | Latest | C++ headers only — free from AMD |
| 🔑 **Administrator** | — | Required for voltage writes to the driver |
...
> **Important:** Read all steps before starting. Total time is ~15 minutes on a fresh machine.

---

### Step 1 — Install Python 3.12

> ⚠️ Python 3.14+ breaks PyInstaller exe builds. You must use **Python 3.12**.
...
> If you already have Visual Studio 2019/2022 installed, you're good — just ensure the C++ workload is installed.
...
Close and reopen the terminal after, then verify:
...
`pywin32` — required for the crash logger to read Windows Event Log  
`pyinstaller` — required only if you want to build the `.exe`

---

### Step 5 — Clone the ADLX SDK

The ADLX SDK provides the C++ headers needed to compile the bridge. The runtime `ADLX.dll` is already installed on your PC by AMD Adrenalin.

```cmd
git clone https://github.com/GPUOpen-LibrariesAndSDKs/ADLX.git
```

Note the full path to where you cloned it — you'll need it in the next step.

---

### Step 6 — Build the C++ bridge

Open a **Developer Command Prompt for VS 2022** (search Start menu for it), then:

```cmd
cd ClawVoltridge

cmake -B build -DADLX_SDK_DIR="C:\path	o\ADLX" -A x64

cmake --build build --config Release
```

Replace `C:\path	o\ADLX` with the actual folder where you cloned the SDK.

> ✅ Success looks like: `adlx_bridge.vcxproj -> ...ridgeuild\Releasedlx_bridge.exe`

Copy the compiled bridge to the repo root:

```cmd
copy build\Releasedlx_bridge.exe ..
```
...
`INTERFACE_MGT2_1:1` confirms RDNA4 voltage control is available.
...
> ⚠️ **Right-click your terminal → Run as Administrator** for all voltage operations.
...
Copy `adlx_bridge.exe` into `build\dist\ClawVolt\` alongside the exe, then right-click → Run as Administrator.
<br/>
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f3460,100:1a1a2e&height=3" width="100%"/>
<br/>
...
[14:21:01] INFO  Crash logger started — watching Event Log
...
| `--config my.json` | Load a saved JSON config |
...
│  Status: active  │     ─ ─ ─ threshold lines drawn on graph ─ ─ ─   │
├──────────────────┤                                                   │
│  Live            ├───────────────────────────────────────────────────┤
│  Core Clock ─── │                                                   │
│  Voltage    ─── │     Session Log                                   │
│  Pending →  ─── │     [14:21:01] Connected: AMD Radeon RX 9070 XT   │
│  Changes    ─── │     [14:21:01] Crash logger started               │
...
| Button | Action |
|:---|:---|
| **▶ Start** | Connect to GPU, start monitoring + crash logger |
| **⏹ Stop** | Stop monitoring, restore auto tuning, write session footer to log |
| **💾** | Save full config (all settings + all threshold rows) to JSON |
| **📂** | Load JSON preset — completely rebuilds all threshold rows |
...
```jsonc
{
  "bridge_path": "adlx_bridge.exe",   // path to compiled bridge binary
  "poll_interval_sec": 0.5,           // clock poll frequency (0.3–1.0s recommended)
  "hysteresis_count": 2,              // reads required before committing a change
  "idle_voltage_offset_mv": -100,     // offset when below all thresholds
  "thresholds": [
    { "clock_mhz": 3200, "voltage_offset_mv": -120 },
    { "clock_mhz": 3100, "voltage_offset_mv": -160 },
    { "clock_mhz": 3000, "voltage_offset_mv": -140 }
  ],
  "verbose": true                     // CLI: print status on every poll tick
}
```

Save a template:
```cmd
python src/claw_volt_cli.py --save-config my_config.json
```

Load it:
```cmd
python src/claw_volt_cli.py --config my_config.json
```
...
| 3100 MHz | −140 mV | High sustained — more headroom safe |
| 3000 MHz | −160 mV | Mid boost — safe to push further |
| idle | −100 mV | Low load — conservative, negligible impact |

</div>

> **Tuning method:** Start at −80 mV everywhere. Run Unigine Heaven for 20 min. Stable? Lower one threshold by 10 mV. Crash? Raise that threshold by 20 mV. Repeat.
...
Every poll tick
  └─► TelemetryPoint(clock, voltage, temp, hotspot, power, fan)
        └─► appended to 300-sample circular deque  (~150s history)

Every 2 seconds (background thread)
  └─► query Windows Event Log
        ├─► Event 4101 (TDR recovery)
        ├─► Event 41   (kernel crash / BSOD)
        └─► Event 6008 (unexpected shutdown)
              └─► new event? → analyse telemetry → write crash report
```
...
| `UNKNOWN` | No pattern — raw telemetry still preserved |
...
```
════════════════════════════════════════════════════════════════════════
  CLAWVOLT CRASH REPORT #1
  Detected : 2025-03-17 14:22:05
  Event    : Windows Event ID 4101 — Display driver stopped responding
════════════════════════════════════════════════════════════════════════
  ...
  ANALYSIS
  ─────────────────────────────────────────────────────────────────────
  Clock was 3142 MHz with a voltage offset of -160 mV at crash time.
  A voltage change to -160 mV occurred 3.2s before the crash.
  Recommendation: raise the ≥3100 MHz threshold by 20-40 mV.

  ACTIVE CONFIG AT CRASH TIME
  Thresholds: ≥3200→-120mV  ≥3100→-160mV  ≥3000→-140mV

  VOLTAGE CHANGE HISTORY (last 60s)
  14:21:59  -140 mV → -160 mV   ← 3.2s before crash

  TELEMETRY SNAPSHOT (last 30 samples)
       Time     Clock   Voltage    Temp   HotSpot    Power    Fan
  ─────────────────────────────────────────────────────────────────────
  14:21:58.0   3124 MHz  -140 mV   72°     85°    218W    63%
  14:21:58.5   3142 MHz  -160 mV   73°     86°    220W    64%  ← crash

  RECOMMENDATIONS
  1. Raise the voltage offset for ≥3100 MHz by 20-40 mV.
  2. Increase hysteresis_count to 3 for more stable transitions.
  3. Test with Unigine Heaven after each change.
════════════════════════════════════════════════════════════════════════
```
...
IADLXSystem
  └─ GetGPUTuningServices()
       └─ IADLXGPUTuningServices
            ├─ IsSupportedManualGFXTuning(gpu)     // check support
...
On RDNA4:
- The range ADLX reports is **−200 mV to 0 mV**
- `0` = stock (no change from AMD defaults)
- `−200` = maximum the driver allows
- The value passed to `SetGPUVoltage` is the offset directly — no arithmetic needed

</details>

<details>
<summary>Click to expand — Compiled source files</summary>

| File | Role |
|:---|:---|
| `adlx_bridge.cpp` | Application logic |
| `SDK/ADLXHelper/Windows/Cpp/ADLXHelper.cpp` | Runtime-loads `ADLX.dll` via `LoadLibrary` |
| `SDK/Platform/Windows/WinAPIs.cpp` | Platform functions: `adlx_load_library`, `adlx_free_library`, `adlx_get_proc_address` |

`ADLX.dll` is **not** linked at compile time — loaded dynamically at runtime. The binary works on any PC with a recent AMD Adrenalin driver installed.
...
<details>
<summary><b>adlx_bridge.exe --info returns nothing / ADLX init failed</b></summary>
...
- Close MSI Afterburner, AMD Adrenalin overlay, GPU Tweak, or any tool holding the tuning lock
...
- Verify no other application is using Adrenalin's Performance Tuning panel simultaneously
...
- Check the Session Log panel in the GUI for error messages
- Test manually: `adlx_bridge.exe --get-clock` should print `CLOCK:XXXX`

</details>

<details>
<summary><b>Preset load doesn't update threshold rows</b></summary>

- Make sure you are using the latest `claw_volt_gui.py`
- Early versions had a bug where only scalar settings were updated on load; this has been fixed
...
| Error | Fix |
|:---|:---|
| `ADLX SDK not found at ...` | `ADLX_SDK_DIR` must point to the repo root (containing `SDK\`) |
| `unresolved external: adlx_load_library` | `WinAPIs.cpp` missing from build — use the provided `CMakeLists.txt` |
| `Cannot open include file: IGPUManualGFXTuning1.h` | This file doesn't exist — all interfaces are in `IGPUManualGFXTuning.h` |
...
|:---|:---|
| 🚫 **Positive offsets blocked** | Bridge code rejects any offset > 0 — cannot accidentally overvolt |
| 📐 **Hardware range clamped** | All writes clamped to the GPU's ADLX-reported range (−200 to 0 mV) |
| 🔄 **Always restores on exit** | Ctrl+C, window close, and Stop all call `ResetToFactory` before exiting |
| 📖 **Crash logger is read-only** | Only reads Event Log and telemetry — never writes voltages itself |
...
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00ccff,50:0f3460,100:1a1a2e&height=120&section=footer" width="100%"/>
